...
\hypersetup{
  ...
  pdfauthor={Suzanne Embury and the software engineering team},
  hidelinks,
...
We expect that the web-based version of this manual will be the one you'll use most at \href{https://software-eng.netlify.app/}{software-eng.netlify.app}. You can search, browse and link to anything in the manual. It was last updated on 17 August, 2022.
...
\texttt{Document\ version:} 17 August, 2022
...
\textasciitilde{}\textasciitilde{}**8**: 15th Nov\textasciitilde{}\textasciitilde{} & \textasciitilde{}\textasciitilde{}Software refactoring, see see section \ref{week8}\textasciitilde{}\textasciitilde{} & \textasciitilde{}\textasciitilde{}Mentoring week 2\textasciitilde{}\textasciitilde{}\\
\textasciitilde{}\textasciitilde{}**9**: 22nd Nov\textasciitilde{}\textasciitilde{} & \textasciitilde{}\textasciitilde{}Design for testability, see see section \ref{week9}\textasciitilde{}\textasciitilde{} & \\
\textasciitilde{}\textasciitilde{}**10**: 29th Nov\textasciitilde{}\textasciitilde{} & \textasciitilde{}\textasciitilde{}Design patterns, see see section \ref{week10}\textasciitilde{}\textasciitilde{} & \textasciitilde{}\textasciitilde{}TeamCwk2, Fri 3rd Dec, 6pm\textasciitilde{}\textasciitilde{}\\
\addlinespace
\textasciitilde{}\textasciitilde{}**11**: 6th Dec\textasciitilde{}\textasciitilde{} & \textasciitilde{}\textasciitilde{}Risk management and practice exam, see see section \ref{week11}\textasciitilde{}\textasciitilde{} & \\
\textasciitilde{}\textasciitilde{}**12**: 13th Dec\textasciitilde{}\textasciitilde{} & \textasciitilde{}\textasciitilde{}Open source challenge, see see section \ref{week12}\textasciitilde{}\textasciitilde{} & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1242}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1242}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1242}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1615}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1615}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1615}}@{}}
\toprule()
...
We're not going to be making any serious changes to the Marauroa code base in this workshop, so we will just ask for a remote tracking branch to be created for the \texttt{master} branch of the repository, see figure \ref{fig:selectMarauroaBranches-fig}. If you need to remind yourself how branching works, you might like to visit (or revisit) \href{https://learngitbranching.js.org/}{learngitbranching.js.org}.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{\textless{}html} \ErrorTok{destdir}\OtherTok{=}\StringTok{"$\{build{-}coverage\}"} \KeywordTok{/\textgreater{}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DataTypeTok{int}\NormalTok{ x }\OperatorTok{=} \BuiltInTok{Math}\OperatorTok{.}\FunctionTok{random}\OperatorTok{();}
...
\begin{Highlighting}[]
\DataTypeTok{int}\NormalTok{ x }\OperatorTok{=} \BuiltInTok{Math}\OperatorTok{.}\FunctionTok{random}\OperatorTok{();}
\FunctionTok{coverage\_log}\OperatorTok{(}\StringTok{"int x = Math.random();"}\OperatorTok{);}
\BuiltInTok{System}\OperatorTok{.}\FunctionTok{out}\OperatorTok{.}\FunctionTok{println}\OperatorTok{(}\NormalTok{x}\OperatorTok{);}
\FunctionTok{coverage\_log}\OperatorTok{(}\StringTok{"System.out.println(x);"}\OperatorTok{);}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{marauroa}\OperatorTok{.}\FunctionTok{common}\OperatorTok{.}\FunctionTok{game}\OperatorTok{.}\FunctionTok{RPSlot}\OperatorTok{.}\FunctionTok{setDeletedRPObject}\OperatorTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{    \textless{}}\KeywordTok{target}\OtherTok{ name=}\StringTok{"all{-}tests{-}report"}\OtherTok{ depends=}\StringTok{"test"}\NormalTok{\textgreater{}}
\NormalTok{        \textless{}}\KeywordTok{property}\OtherTok{ name=}\StringTok{"test{-}summary{-}report"}\OtherTok{ value=}\StringTok{"$\{build{-}tests{-}report\}/summary"}\NormalTok{/\textgreater{}}
\NormalTok{        \textless{}}\KeywordTok{mkdir}\OtherTok{ dir=}\StringTok{"$\{test{-}summary{-}report\}"}\NormalTok{/\textgreater{}}
\NormalTok{        \textless{}}\KeywordTok{junitreport}\OtherTok{ todir=}\StringTok{"$\{test{-}summary{-}report\}"}\NormalTok{\textgreater{}}
\NormalTok{            \textless{}}\KeywordTok{fileset}\OtherTok{ dir=}\StringTok{"$\{build{-}tests{-}report\}"}\NormalTok{\textgreater{}}
\NormalTok{                \textless{}}\KeywordTok{include}\OtherTok{ name=}\StringTok{"TEST{-}*.xml"}\NormalTok{/\textgreater{}}
\NormalTok{            \textless{}/}\KeywordTok{fileset}\NormalTok{\textgreater{}}
\NormalTok{            \textless{}}\KeywordTok{report}\OtherTok{ format=}\StringTok{"frames"}\OtherTok{ todir=}\StringTok{"$\{test{-}summary{-}report\}"}\NormalTok{/\textgreater{}}
\NormalTok{        \textless{}/}\KeywordTok{junitreport}\NormalTok{\textgreater{}}
\NormalTok{    \textless{}/}\KeywordTok{target}\NormalTok{\textgreater{}}
\end{Highlighting}
\end{Shaded}
...
In this workshop, we will look at strategies you can use to better comprehend unfamiliar large codebases. These strategies are supported by code reading techniques \citep{codereading} and the functionalities offered by your Integrated Development Environment (IDE). We'll be assuming that, after this workshop, you will be capable of carrying out the following tasks for yourself, without needing much guidance:
...
\begin{Highlighting}[]
\AttributeTok{@Test}
\KeywordTok{public} \DataTypeTok{void} \FunctionTok{testReverse} \OperatorTok{()} \OperatorTok{\{}
    \CommentTok{// Arrange}
    \BuiltInTok{String}\NormalTok{ input }\OperatorTok{=} \StringTok{"abc"}\OperatorTok{;}
\NormalTok{    StringUtils sut }\OperatorTok{=} \KeywordTok{new} \FunctionTok{StringUtils}\OperatorTok{();} \CommentTok{// sut = system under test}

    \CommentTok{// Act}
    \BuiltInTok{String}\NormalTok{ result }\OperatorTok{=}\NormalTok{ sut}\OperatorTok{.}\FunctionTok{reverse}\OperatorTok{(}\NormalTok{input}\OperatorTok{);}

    \CommentTok{// Assert}
    \FunctionTok{assertEquals}\OperatorTok{(}\StringTok{"cba"}\OperatorTok{,}\NormalTok{ result}\OperatorTok{);}
...
\begin{Shaded}
\begin{Highlighting}[]
\BuiltInTok{String}\NormalTok{ input}\OperatorTok{;}
\NormalTok{StringUtils sut}\OperatorTok{;} \CommentTok{// sut = system under test}

\CommentTok{// this method will be used in the arrange section of the tests}
\KeywordTok{private} \DataTypeTok{void} \FunctionTok{initialize}\OperatorTok{()\{}
\NormalTok{    input }\OperatorTok{=} \StringTok{"abc"}\OperatorTok{;}
\NormalTok{    sut }\OperatorTok{=} \KeywordTok{new} \FunctionTok{StringUtils}\OperatorTok{();}
...
\KeywordTok{public} \DataTypeTok{void} \FunctionTok{testReverse} \OperatorTok{()} \OperatorTok{\{}
    ...
    \CommentTok{// Arrange}
    \FunctionTok{initialize}\OperatorTok{();} \CommentTok{// everything we need is arranged using this single line}
...
\begin{Shaded}
\begin{Highlighting}[]
\AttributeTok{@Test}\OperatorTok{(}\NormalTok{expected }\OperatorTok{=} \BuiltInTok{Exception}\OperatorTok{.}\FunctionTok{class}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\AttributeTok{@Test}\OperatorTok{(}\NormalTok{timeout}\OperatorTok{=}\DecValTok{100}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{fail}\OperatorTok{(}\BuiltInTok{String}\NormalTok{ message}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assertTrue}\OperatorTok{(}\BuiltInTok{String}\NormalTok{ message}\OperatorTok{,} \DataTypeTok{boolean}\NormalTok{ condition}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assertFalse}\OperatorTok{(}\BuiltInTok{String}\NormalTok{ message}\OperatorTok{,} \DataTypeTok{boolean}\NormalTok{ condition}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assertEquals}\OperatorTok{(}\BuiltInTok{String}\NormalTok{ message}\OperatorTok{,}\NormalTok{ expected}\OperatorTok{,}\NormalTok{ actual}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assertEquals}\OperatorTok{(}\BuiltInTok{String}\NormalTok{ message}\OperatorTok{,}\NormalTok{ expected}\OperatorTok{,}\NormalTok{ actual}\OperatorTok{,}\NormalTok{ tolerance}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assertNull}\OperatorTok{(}\BuiltInTok{String}\NormalTok{ message}\OperatorTok{,}\NormalTok{ object}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assertNotNull}\OperatorTok{(}\BuiltInTok{String}\NormalTok{ message}\OperatorTok{,}\NormalTok{ object}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assertSame}\OperatorTok{(}\BuiltInTok{String}\NormalTok{ message}\OperatorTok{,}\NormalTok{ expected}\OperatorTok{,}\NormalTok{ actual}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{assertNotSame}\OperatorTok{(}\BuiltInTok{String}\NormalTok{ message}\OperatorTok{,}\NormalTok{ expected}\OperatorTok{,}\NormalTok{ actual}\OperatorTok{)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{wbs}{%
\section{Work Breakdown Structures}\label{wbs}}
...
{\centering \includegraphics[width=1\linewidth]{images/cost-estimation-video} 

}

\caption{Work Breakdown Structures (WBS) will help to improve the accuracy and justifiability of your cost estimations in software engineering. The image in the picture is a screenshot, to watch the full seven minute video at \href{https://youtu.be/tKCfnF-z2hY}{youtu.be/tKCfnF-z2hY}}\label{fig:wbs-video-fig}
\end{figure}



For example, in stendhal, lets say you needed to estimate the cost if fixing a bug about players losing health points between 5.00am and 7.00am in the morning. This is \texttt{100\%} of your task that you could break this down into several sub-tasks follows:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Replicate the bug (\texttt{20\%})
\item
  Fix the bug (\texttt{20\%})
\item
  Check that the bug is really fixed (\texttt{20\%})
\item
  Push the bug fix to the repository (\texttt{20\%})
\item
  Make sure the same bug doesn't exist elsewhere (\texttt{20\%})
\end{enumerate}
...
\item
  ...
  \tightlist
  \item
    Replicate the bug manually (\texttt{5\%})
  \item
    Gather missing info from reporter (\texttt{5\%})
  \item
    Find tests for this or similar functionality (\texttt{5\%})
  \item
    Write test that reveals the bug (\texttt{5\%})
  \end{enumerate}
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tests}\OperatorTok{/}\NormalTok{games}\OperatorTok{/}\NormalTok{stendhal}\OperatorTok{/}\NormalTok{client}\OperatorTok{/}\NormalTok{sound}\OperatorTok{/}\NormalTok{system}\OperatorTok{/}\NormalTok{ToneGeneratorTest}\OperatorTok{.}\FunctionTok{java}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{src}\OperatorTok{/}\NormalTok{games}\OperatorTok{/}\NormalTok{stendhal}\OperatorTok{/}\NormalTok{client}\OperatorTok{/}\NormalTok{sound}\OperatorTok{/}\NormalTok{system}\OperatorTok{/}\NormalTok{processors}\OperatorTok{/}\NormalTok{ToneGenerator}\OperatorTok{.}\FunctionTok{java}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{src}\OperatorTok{/}\NormalTok{games}\OperatorTok{/}\NormalTok{stendhal}\OperatorTok{/}\NormalTok{server}\OperatorTok{/}\NormalTok{actions}\OperatorTok{/}\NormalTok{pet}\OperatorTok{/}\NormalTok{OwnAction}\OperatorTok{.}\FunctionTok{java}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{src}\OperatorTok{/}\NormalTok{games}\OperatorTok{/}\NormalTok{stendhal}\OperatorTok{/}\NormalTok{server}\OperatorTok{/}\NormalTok{actions}\OperatorTok{/}\NormalTok{pet}\OperatorTok{/}\NormalTok{NameAction}\OperatorTok{.}\FunctionTok{java}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{src}\OperatorTok{/}\NormalTok{games}\OperatorTok{/}\NormalTok{stendhal}\OperatorTok{/}\NormalTok{server}\OperatorTok{/}\NormalTok{actions}\OperatorTok{/}\NormalTok{equip}\OperatorTok{/}\NormalTok{EquipAction}\OperatorTok{.}\FunctionTok{java}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tests}\OperatorTok{/}\NormalTok{games}\OperatorTok{/}\NormalTok{stendhal}\OperatorTok{/}\NormalTok{server}\OperatorTok{/}\NormalTok{maps}\OperatorTok{/}\NormalTok{quests}\OperatorTok{/}\NormalTok{IcecreamForAnnieTest}\OperatorTok{.}\FunctionTok{java}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{/}\NormalTok{stendhal}\OperatorTok{/}\NormalTok{tests}\OperatorTok{/}\NormalTok{games}\OperatorTok{/}\NormalTok{stendhal}\OperatorTok{/}\NormalTok{server}\OperatorTok{/}\NormalTok{maps}\OperatorTok{/}\NormalTok{quests}\OperatorTok{/}\NormalTok{FindRatChildrenTest}\OperatorTok{.}\FunctionTok{java}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{games}\OperatorTok{.}\FunctionTok{stendhal}\OperatorTok{.}\FunctionTok{server}\OperatorTok{.}\FunctionTok{core}\OperatorTok{.}\FunctionTok{config}\OperatorTok{.}\FunctionTok{zone}\OperatorTok{.}\FunctionTok{NoTeleportInTest}\OperatorTok{.}\FunctionTok{testConfigureZone}\OperatorTok{()\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{games}\OperatorTok{.}\FunctionTok{stendhal}\OperatorTok{.}\FunctionTok{client}\OperatorTok{.}\FunctionTok{gui}\OperatorTok{.}\FunctionTok{ItemPanelTest}\OperatorTok{.}\FunctionTok{testCursors}\OperatorTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{games}\OperatorTok{.}\FunctionTok{stendhal}\OperatorTok{.}\FunctionTok{client}\OperatorTok{.}\FunctionTok{entity}\OperatorTok{.}\FunctionTok{EntityTest}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{games}\OperatorTok{.}\FunctionTok{stendhal}\OperatorTok{.}\FunctionTok{server}\OperatorTok{.}\FunctionTok{actions}\OperatorTok{.}\FunctionTok{admin}\OperatorTok{.}\FunctionTok{SummonActionTest}\OperatorTok{.}\FunctionTok{setUP}\OperatorTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{games}\OperatorTok{.}\FunctionTok{stendhal}\OperatorTok{.}\FunctionTok{server}\OperatorTok{.}\FunctionTok{entity}\OperatorTok{.}\FunctionTok{creature}\OperatorTok{.}\FunctionTok{impl}\OperatorTok{.}\FunctionTok{attack}\OperatorTok{.}\FunctionTok{HandToHandTest}\OperatorTok{.}\FunctionTok{java}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{public} \DataTypeTok{void} \FunctionTok{attack}\OperatorTok{(}\DataTypeTok{final}\NormalTok{ Creature creature}\OperatorTok{)} \OperatorTok{\{}
    \ControlFlowTok{if} \OperatorTok{(}\NormalTok{creature}\OperatorTok{.}\FunctionTok{isAttackTurn}\OperatorTok{(}\NormalTok{SingletonRepository}\OperatorTok{.}\FunctionTok{getRuleProcessor}\OperatorTok{().}\FunctionTok{getTurn}\OperatorTok{()))\{}
...
{\centering \href{https://youtube.com/embed/H3rnYhd5hx8}{\includegraphics[width=1\linewidth]{softeng_files/figure-latex/mentoring-fig-1} }

}

\caption{An outline of software engineering mentoring at the University of Manchester. You can also watch the 13 minute introductory video for mentors at \href{https://youtu.be/H3rnYhd5hx8}{youtu.be/H3rnYhd5hx8}}\label{fig:mentoring-fig}
\end{figure}
...
You will document your release plans by placing them in the wiki of your team's GitLab project. You should create a page in the wiki for each issue, using the page slugs shown on Blackboard (under \texttt{Assessment} \textgreater{} \texttt{Team\ Coursework} \textgreater{} \texttt{Exercise\ 2}. For example, \textbf{for team S1Team107}, if the page slug for one issue is given as \texttt{slow-innkeeper}, then the wiki page for this issue would be reached at the following URL:
...
\texttt{NOTE:\ YOU\textquotesingle{}LL\ NEED\ TO\ SUBSTITUTE\ YOUR\ TEAM\ NUMBER\ INTO\ THE\ URL\ ABOVE}
...
⚠️ \textbf{Caution} ⚠️
Please make sure you set the \texttt{user.name} and \texttt{user.email} variables on all machines you intend to commit from. Your email needs to you \texttt{firstname.lastname@student.manchester.ac.uk} address.

\textbf{Commits made from other accounts will NOT be considered to be part of your team's submission for the exercise!}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{import} \KeywordTok{static}\ImportTok{ org}\OperatorTok{.}\ImportTok{hamcrest}\OperatorTok{.}\ImportTok{MatcherAssert}\OperatorTok{.}\ImportTok{assertThat}\OperatorTok{;}
\KeywordTok{import} \KeywordTok{static}\ImportTok{ org}\OperatorTok{.}\ImportTok{hamcrest}\OperatorTok{.}\ImportTok{core}\OperatorTok{.}\ImportTok{Is}\OperatorTok{.}\ImportTok{is}\OperatorTok{;}

\KeywordTok{import} \ImportTok{org}\OperatorTok{.}\ImportTok{junit}\OperatorTok{.}\ImportTok{Test}\OperatorTok{;}

\KeywordTok{public} \KeywordTok{class}\NormalTok{ LargestSquareTest }\OperatorTok{\{}

    \AttributeTok{@Test}
    \KeywordTok{public} \DataTypeTok{void} \FunctionTok{shouldReturn0AsLargestSquareLessThanOrEqualTo0}\OperatorTok{()} \OperatorTok{\{}
        \FunctionTok{assertThat}\OperatorTok{(}\NormalTok{LargestSquare}\OperatorTok{.}\FunctionTok{lessThanOrEq}\OperatorTok{(}\DecValTok{0}\OperatorTok{),} \FunctionTok{is}\OperatorTok{(}\DecValTok{0}\OperatorTok{));}
    \OperatorTok{\}}

    \AttributeTok{@Test}
    \KeywordTok{public} \DataTypeTok{void} \FunctionTok{shouldReturn1AsLargestSquareLessThanOrEqualTo1}\OperatorTok{()} \OperatorTok{\{}
        \FunctionTok{assertThat}\OperatorTok{(}\NormalTok{LargestSquare}\OperatorTok{.}\FunctionTok{lessThanOrEq}\OperatorTok{(}\DecValTok{1}\OperatorTok{),} \FunctionTok{is}\OperatorTok{(}\DecValTok{1}\OperatorTok{));}
    \OperatorTok{\}}

    \AttributeTok{@Test}
    \KeywordTok{public} \DataTypeTok{void} \FunctionTok{shouldReturn1AsLargestSquareLessThanOrEqualTo3}\OperatorTok{()} \OperatorTok{\{}
        \FunctionTok{assertThat}\OperatorTok{(}\NormalTok{LargestSquare}\OperatorTok{.}\FunctionTok{lessThanOrEq}\OperatorTok{(}\DecValTok{3}\OperatorTok{),} \FunctionTok{is}\OperatorTok{(}\DecValTok{1}\OperatorTok{));}
    \OperatorTok{\}}

    \AttributeTok{@Test}
    \KeywordTok{public} \DataTypeTok{void} \FunctionTok{shouldReturn4AsLargestSquareLessThanOrEqualTo4}\OperatorTok{()} \OperatorTok{\{}
        \FunctionTok{assertThat}\OperatorTok{(}\NormalTok{LargestSquare}\OperatorTok{.}\FunctionTok{lessThanOrEq}\OperatorTok{(}\DecValTok{4}\OperatorTok{),} \FunctionTok{is}\OperatorTok{(}\DecValTok{4}\OperatorTok{));}
    \OperatorTok{\}}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
