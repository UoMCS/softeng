...
We expect that the web-based version of this manual will be the one you'll use most at \href{https://software-eng.netlify.app/}{software-eng.netlify.app}. You can search, browse and link to anything in the manual. It was last updated on 05 October, 2021.
...
\item
  \href{https://cgi.csc.liv.ac.uk/~tomc/}{Thomas Carroll}
\item
...
Document version: 05 October, 2021
...
Most teams have a mixture of online and on campus students. If you're on campus go to the rooms below and contact your team via teams:
...
\chapter{Testing Stendhal}\label{starting}}
...
\href{https://stendhalgame.org/wiki/Stendhal:Administration}{stendhalgame.org/wiki/Stendhal:Administration}
...
\hypertarget{testings}{%
\chapter{Building Stendhal}\label{testings}}
...
\tightlist
\item
  Increasing visibility of your employer as an organisation that makes good quality software
\item
...
Your mentor is a professional software engineer from one of the employers listed here: Apadmi, ARM, Auto Trader UK, Barclays, the BBC, Beyond Trust, Blaize, Bloomberg, Booking.com, CDL Software, CodeThink, Code Computer Love, Cubic Motion (now Epic Games), DataCentred, Digital Bridge Ltd, Disney Streaming, EGN Digital, Facebook, Farm Digital, Giant Digital, Goldman Sachs, Google, IBM, Ivanti, Koder.ly, Matillion, Morgan Stanley, NCC Group, On The Beach, Rental Cars, Sainsburys, Sage Group plc, Siemens (Mentor Graphics), SteamaCo, THG, ThoughtWorks and Zuhlke.
...
\hypertarget{but-im-not-interested-in-employer-x}{%
\section{But I'm not interested in employer x?}\label{but-im-not-interested-in-employer-x}}
...
This second individual coursework exercise for COMP23311 is designed to help you develop two new skills that professional software engineers need, and that will be essential for the remaining team coursework:
...
\caption{\label{tab:marktable2}The Mark scheme for the second piece of individual coursework}
\centering
\begin{tabular}[t]{ll}
\toprule
Criterion & Marks\\
...
When all the tests on the \texttt{move-counter} branch pass, this step is completed.
...
You can now make a commit containing the details of your implementation. If you want to make multiple commits, that's fine, too. But the commits \emph{must} appear on the \texttt{move-counter} branch.
...
This step is complete when you can see all your changes and commits on GitLab, and when the \texttt{move-counter} branch on GitLab is pointing to your latest commit.
...
Start by checking out the branch that you want to include the new feature: in this case, \texttt{master}.

In the \texttt{History} view of your project, right click on the commit that the \texttt{move-counter} branch is pointing at, and select \texttt{Merge}.
...
\caption{Merge to Master Menu}\label{fig:mergeFBtoMasterMenu-fig}
\end{figure}
...
⚠️ \textbf{Caution} ⚠️
The point of this exercise is to walk through the experience of detecting and resolving conflicts between two independent lines of development, and to understand how Git helps us to manage those conflicts. If you followed the natural conventions and approaches when implementing the Move Counter feature, you will discover conflicts when you try to merge. If you have found some clever way to implement the feature that avoids conflicts at this merge (e.g.~by rebasing the feature branch onto \texttt{master} before starting the implementation of the feature), you'll need to think again. You can't get the bulk of the marks for this exercise unless you implement the feature in a way that causes conflicts.

\hypertarget{resolveconfcomp}{%
\subsection{Step 6: Resolve the Conflicts and Complete the Merge}\label{resolveconfcomp}}

If you look at the \texttt{History} view after the merge attempt of the previous step, you will notice that no merge commit was created, and the \texttt{move-counter} branch is still shown as being separate from and unconnected with the \texttt{master} branch.
...
The editor window shows this Git markup. It consists of three horizontal lines, separating the different versions of the code. In this case, the top bars show the code as it is in the currently-checked-out (unmerged) \texttt{master} branch. (You will recall that \texttt{HEAD} is a shorthand branch name, referring to the currently checked out branch.) Below the middle bar is the version of this line of code as it presently is in the \texttt{move-counter} branch.
...
The Merge Tool shows both versions of the code, side-by-side, with the changed lines colour coded for easy detection of places where attention is needed. The left hand window shows the version of the code in \texttt{master}, while the right hand window shows the version of the code in \texttt{move-counter}. The coloured bars between the windows show the changes: grey lines are present in \texttt{master} but are not in \texttt{move-counter}, purple lines are present in \texttt{move-counter} but are not in \texttt{master}, while red lines are present in both versions and are in conflict.
...
Our task is to edit the file in the left hand window, so that it contains the desired integration of the changes in both files. in this case, it's an easy change: we need to include both lines unchanged, so we copy and paste the line from the \texttt{move-counter} window into the \texttt{master} window. Then we save the file.
...
Both ways of removing the conflict also removed the compilation error from the file, and that decorator has therefore been removed from the file in the \texttt{Package\ Explorer} view. The conflict marker remains on the file, however, as we have not yet told Git that we believe the conflicts have been removed.
...
\textbf{For this coursework, we ask that you add some extra information about how you handled the conflicts, but that you leave this automatically generated message unchanged, at the top.}

The local history view of the project now shows that the feature branch has been merged into \texttt{master}, with a merge commit. The resulting code version contains both features, integrated together.
...
After the complexities of the previous step, this one is very easy. Just push the new location of the \texttt{master} branch, plus the merge commit on it, to the remote, following the same instructions as for individual coursework exercise 1 in chapter \ref{gitting},
...
Following the same process covered in the workshops, each team member should acquire their own local copy of the team repository, learn how to build and test it, and run the code as a locally hosted version of the game. Instructions for doing this are in chapter \ref{testings}.
