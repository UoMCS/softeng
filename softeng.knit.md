...
# date: "last updated 10 October, 2021"
...
\caption{(ref:openingcaption)}(\#fig:course-overview-fig)
\end{figure}
...
We expect that the web-based version of this manual will be the one you'll use most at [software-eng.netlify.app](https://software-eng.netlify.app/). You can search, browse and link to anything in the manual. It was last updated on 10 October, 2021.
...
\caption{(ref:captionccbyncnd)}(\#fig:cc-by-nc-nd-fig)
\end{figure}
...
Document version: 10 October, 2021
...
\caption{(ref:captionxkcdcompiling)}(\#fig:xkcd-compiling-fig)
\end{figure}
...
\caption{(ref:captionlonewolf)}(\#fig:wolf-fig)
\end{figure}
...
The academic staff on this course include Suzanne Embury, Anas Elhag, Duncan Hull, Thomas Carroll and Sandra Sampaio. We're here to help but please remember, there are 400+ students on this course so we can't reply to every single personal email.
...
Intead, please post issues on the forum at (ref:piazzaforum) where *everyone* can see the response or in the help queue (ref:livehelpqueue) within the workshop / team study sessions.
:::

we're often very busy and have other teaching commitments too besides this course. We're here to ensure that the course runs smoothly and we aim to give feedback on coursework to you within the two week window.
...
\caption{(\#tab:schedtable)The weekly schedule for this twelve week course, please note we are using the week numbering from the [timetables.manchester.ac.uk](https://timetables.manchester.ac.uk/) where week zero is welcome week, and week one is the first teaching week}
\centering
\begin{tabular}[t]{lll}
\toprule
Week no. & Subject & Deadlines\\
\midrule
\textasciitilde{}\textasciitilde{}**1**: 27th Sept\textasciitilde{}\textasciitilde{} & \textasciitilde{}\textasciitilde{}Automated build and test, see (ref:weekone)\textasciitilde{}\textasciitilde{} & \textasciitilde{}\textasciitilde{}IndCwk2, Fri 1st Oct, 6pm\textasciitilde{}\textasciitilde{}\\
...
\caption{Your main eclipse window should look something like this}(\#fig:firstview-fig)
\end{figure}
...
\caption{Your main eclipse window should look something like this}(\#fig:eclipseImportProjectFromGit-fig)
\end{figure}
...
\caption{Your main eclipse window should look something like this}(\#fig:eclipseCloneFromURI-fig)
\end{figure}
...
\caption{Your Import Projects from Git dialogue box should look like this}(\#fig:enterMarauroaURI-fig)
\end{figure}
...
We're not going to be making any serious changes to the Marauroa code base in this workshop, so we will just ask for a remote tracking branch to be created for the `master` branch of the repository, see figure \@ref(fig:selectMarauroaBranches-fig).
...
\caption{Take a look at the list of branches contained in the project, by scrolling up and down the list. You'll see that the Marauroa project uses separate branches to describe specific releases, as well as other development branches. Another common approach is to have a single release branch and to use tags to distinguish specific releases on that branch.}(\#fig:selectMarauroaBranches-fig)
\end{figure}
...
\caption{Cloning from git dialog box}(\#fig:configureLocalStorageMarauroa-fig)
\end{figure}
...
\caption{Cloning from git dialog box}(\#fig:wizardImport-fig)
\end{figure}
...
\caption{Import projects from a git repository and the newmarauroa project}(\#fig:eclipseSelectProjectsToImport-fig)
\end{figure}
...
\caption{Import projects from a git repository and the newmarauroa project}(\#fig:finallyImportedProject-fig)
\end{figure}
...
\caption{(ref:captiondetach)}(\#fig:detachedHEAD-fig)
\end{figure}
...
\caption{An XML build file}(\#fig:buildfile-fig)
\end{figure}
...
\caption{An XML build file}(\#fig:antAddBuildfile-fig)
\end{figure}
...
\caption{Buildfile Selection}(\#fig:selectBuildfile-fig)
\end{figure}
...
\caption{The jar-all target}(\#fig:jarallTarget-fig)
\end{figure}
...
\caption{Console output}(\#fig:consoleOut-fig)
\end{figure}
...
\caption{Console output}(\#fig:endconsoleOut-fig)
\end{figure}
...
\caption{(ref:captionshowtestcaption)}(\#fig:showTestTargetInBuildScript-fig)
\end{figure}
...
\caption{Testing targets}(\#fig:compileTestsTargetJustCode-fig)
\end{figure}
...
\caption{The testreport}(\#fig:showJUnitTestResultsInXMLFormat-fig)
\end{figure}
...
\caption{The JUnit view}(\#fig:showTestResultsForClientCoverage-fig)
\end{figure}
...
\caption{The Editor view}(\#fig:showSourceOfTestFromTestReport-fig)
\end{figure}
...
\caption{The JUnit view}(\#fig:addHTMLreportRequestToBuildFile-fig)
\end{figure}
...
\caption{A test report}(\#fig:showCoverageReport-fig)
\end{figure}
...
\caption{A more detailed coverage report}(\#fig:showDetailedMethodCoverageReportMac-fig)
\end{figure}
...
\caption{A more detailed coverage report}(\#fig:commentOutALineOfCode-fig)
\end{figure}
...
\caption{A more detailed coverage report}(\#fig:resultsOfRunningTheTestsOnTheModifiedCode-fig)
\end{figure}
...
\caption{Tests FAILED}(\#fig:resultsOfRunningTheTestsOnTheModifiedCodeCompleteBuild-fig)
\end{figure}
...
\caption{(ref:captionxkcdlarge)}(\#fig:xkcd-large-fig)
\end{figure}
...
In this workshop, we will look at strategies to better comprehend unfamiliar large codebases. These strategies are supported by code reading techniques [@codereading] and the functionalities offered by your Integrated Development Environment (IDE). We'll be assuming that, after this workshop, you will be capable of carrying out the following tasks for yourself, without needing much guidance:
...
\caption{marauroa is a large codbase, how can you find your way around it?}(\#fig:marauroa-fig)
\end{figure}
...
\caption{A screenshot from marauroa}(\#fig:marauroa2-fig)
\end{figure}
...
\caption{JUnit Test Case, with Test appended to the Acme class}(\#fig:junit-name-fig)
\end{figure}
...
\caption{The Class Under Test in this example is Acme}(\#fig:junit-classundertest-fig)
\end{figure}
...
\caption{(ref:captionxkcddebugging)}(\#fig:xkcd-debugging-fig)
\end{figure}
...
\caption{(ref:captionissue5)}(\#fig:issue5-fig)
\end{figure}
...
\caption{(ref:captionxkcdestimation)}(\#fig:xkcd-estimation-fig)
\end{figure}
...
\caption{(ref:captionxkcdgit)}(\#fig:xkcd-git-fig)
\end{figure}
...
\caption{Pets, Cats, Goats and Magic Dragons}(\#fig:basicpets-fig)
\end{figure}
...
\caption{Possible subclasses of Pet}(\#fig:petsubclasses-fig)
\end{figure}
...
\caption{A badly designed hierarchy}(\#fig:petsubclasses2-fig)
\end{figure}
...
\caption{Your codebase should look something like this at the end of this migration task}(\#fig:petstrategy-fig)
\end{figure}
...
\caption{Subclasses of Habitat}(\#fig:habitat-fig)
\end{figure}
...
\caption{Your codebase should look something like this at the end of this migration task}(\#fig:composite-fig)
\end{figure}
...
\caption{(ref:captionxkcdrisk)}(\#fig:xkcd-risk-fig)
\end{figure}
...
\caption{Create local Stendhal account}(\#fig:createLocalStendhalAccount-fig)
\end{figure}
...
\caption{Login to local Stendhal account}(\#fig:loginToLocalStendhalAccount-fig)
\end{figure}
...
\caption{(ref:captionmentoring)}(\#fig:mentoring-fig)
\end{figure}
...
We suggest the following questions may be useful for structuring your meeting:
...
\caption{Your main eclipse window should look something like this}(\#fig:fileStagedForCommit-fig)
\end{figure}
...
\caption{Your setup should look something like this}(\#fig:localCommitGraphBeforePushNoHistory-fig)
\end{figure}
...
\caption{(ref:captiongitlab)}(\#fig:commitGraphInGitLabBeforePushNoHistory-fig)
\end{figure}
...
\caption{Your commit graph should look something like this}(\#fig:commitGraphInGitLabAfterPushNoHistory-fig)
\end{figure}
...
\caption{Your commit graph should look something like this}(\#fig:teamMatesLocalCommitGraphBeforeFetchNoHistory-fig)
\end{figure}
...
\caption{The fetch results dialog box}(\#fig:fetchSucceededNoHistory-fig)
\end{figure}
...
\caption{An updated History view showing the results of the fetch operation}(\#fig:teamMatesLocalCommitGraphAfterFetchNoHistory-fig)
\end{figure}
...
\caption{Your commit graph should look something like this}(\#fig:teamMatesLocalCommitGraphAfterFFMergeNoHistory-fig)
\end{figure}
...
\caption{(\#tab:marktable)The Mark scheme for the first piece of individual coursework}
\centering
\begin{tabular}[t]{ll}
\toprule
Criterion & Marks\\
...
\caption{Selecting a branch}(\#fig:selectLocalBranchesForClone-fig)
\end{figure}
...
\caption{Import existing Eclipse projects }(\#fig:importExistingEclipseProjects-fig)
\end{figure}
...
\caption{Selecting Eclipse Project to import}(\#fig:selectEclipseProjectToImport-fig)
\end{figure}
...
\caption{An Eclipse project loaded and visible in package explorer}(\#fig:eclipseProjectLoadedAndVisibleInPackageExplorer-fig)
\end{figure}
...
\caption{Viewing the test code}(\#fig:viewingtheTestCode-fig)
\end{figure}
...
\caption{After running the tests you should see some that fail}(\#fig:runTheTestsAndSeeSomeFail-fig)
\end{figure}
...
\caption{After creating branch note the annotation change in the package explorer}(\#fig:afterCreateBranchSeeTheAnnotationChangeInPackageExplorer-fig)
\end{figure}
...
\caption{Git staging view before changes with comment}(\#fig:gitStagingViewBeforeChangesWithComment-fig)
\end{figure}
...
\caption{Some unstaged changes showing in staging view}(\#fig:unstagedChangesShowingInStagingView-fig)
\end{figure}
...
\caption{The JUnit view}(\#fig:runTestsAfterMakingTheChangeAndAllPass-fig)
\end{figure}
...
\caption{Stage the changes now that the tests pass}(\#fig:stageTheChangesNowTheTestsPass-fig)
\end{figure}
...
\caption{check commit in history view}(\#fig:checkCommitInHistoryView-fig)
\end{figure}
...
\caption{Screenshot showing gitlab before pushing}(\#fig:checkGitLabRemoteBeforePush-fig)
\end{figure}
...
\caption{Configure push dialog}(\#fig:configurePushRefSpecs-fig)
\end{figure}
...
\caption{Confirmation of push to remote dialog}(\#fig:confirmationOfPushToRemote-fig)
\end{figure}
...
\caption{Request theM merge into master and select branch to merge in dialog}(\#fig:requestTheMergeIntoMasterAndSelectbranchToMergeIn-fig)
\end{figure}
...
\caption{Screenshot showing merge results}(\#fig:mergeResultShowingSuccessFastForward-fig)
\end{figure}
...
\caption{Screenshot showing check gitlab network after feature branch is pushed}(\#fig:checkGitLabNetworkAfterFeatureBranchIsPushed-fig)
\end{figure}
...
\caption{(\#tab:marktable2)The marking scheme for the 2nd individual coursework exercise}
\centering
\begin{tabular}[t]{ll}
\toprule
Criterion & Marks\\
...
\caption{Merge to Main Menu}(\#fig:mergeFBtoMasterMenu-fig)
\end{figure}
...
\caption{Merge results showing conflicts}(\#fig:mergeResultShowingConflicts-fig)
\end{figure}
...
\caption{Conflicts shown after merge paused}(\#fig:conflictsShownAfterMergePaused-fig)
\end{figure}
...
\caption{Manually edit out conflict}(\#fig:manuallyEditOutConflict-fig)
\end{figure}
...
\caption{Invoking the merge tool}(\#fig:invokingTheMergeTool-fig)
\end{figure}
...
\caption{Merge tool showing conflicts}(\#fig:mergeToolShowingConflicts-fig)
\end{figure}
...
\caption{Edits in merge tool}(\#fig:editsInMergeTool-fig)
\end{figure}
...
\caption{Special commit view}(\#fig:specialCommitView-fig)
\end{figure}
...
\caption{Commit view after GUI staged}(\#fig:commitViewAfterGuiStaged-fig)
\end{figure}
...
\caption{Commit view when all conflicts have been removed}(\#fig:commitViewWhenAllConflictsRemoved-fig)
\end{figure}
...
\caption{Local history after merge completed}(\#fig:localHistoryAfterMergeCompleted-fig)
\end{figure}
...
\caption{Your local history}(\#fig:localCommitGraphBeforeRejectedPushNoHistoryGraphOnly-fig)
\end{figure}
...
\caption{Your remote commit graph}(\#fig:commitGraphInGitLabAfterPushNoHistoryGraphOnly-fig)
\end{figure}
...
\caption{Can you see why git has rejected this push?}(\#fig:pushRejectedNoHistory-fig)
\end{figure}
...
\caption{Your local commit graph}(\#fig:localCommitGraphAfterPushRejectedAndFetchNoHistoryb-fig)
\end{figure}
...
\caption{Merge result dialog box indicating success}(\#fig:mergeResultDialogueAfterNonFFMergeNoHistory-fig)
\end{figure}
...
\caption{The History view shows us the new local commit graph}(\#fig:localCommitGraphAfterNonFFMergeNoHistory-fig)
\end{figure}
...
\caption{New local commit graph with the graph at the remote, still as it was at the start of the scenario}(\#fig:commitGraphInGitLabAfterPushNoHistoryGraphOnly2-fig)
\end{figure}
...
\caption{The remote repository now contains our commits}(\#fig:commitGraphInGitLabAfterNonFFMergePushedNoHistory-fig)
\end{figure}
...
\caption{Git has updated the position of the remote tracking branch}(\#fig:localCommitGraphAfterSuccessfulPushAfterNonFFMergeNoHistory-fig)
\end{figure}
...
\caption{Our local commit graph}(\#fig:localCommitGraphAfterPushRejectedAndFetchNoHistorya-fig)
\end{figure}
...
\caption{Our team remote graph}(\#fig:commitGraphInGitLabAfterPushNoHistoryGraphOnlya-fig)
\end{figure}
...
\caption{A simple commit made by the Stendhal development team}(\#fig:sampleGitLabCommitView-fig)
\end{figure}
...
\caption{Our local commit graph}(\#fig:localCommitGraphMasterRebaseExampleBeforeRebaseNoHistory-fig)
\end{figure}
...
\caption{A commit graph, note the changes carefully}(\#fig:localCommitGraphMasterRebaseExampleAfterRebaseNoHistory-fig)
\end{figure}
...
\caption{Our local graph after we've fetched in the commits from the remote}(\#fig:localCommitGraphFBRebaseExampleBeforeRebaseNoHistory-fig)
\end{figure}
...
\caption{updated commit graph}(\#fig:localCommitGraphFBRebaseExampleAfterMasterRebaseNoHistory-fig)
\end{figure}
...
\caption{New commit graph}(\#fig:localCommitGraphFBRebaseExampleAfterFBRebaseNoHistory-fig)
\end{figure}
...
\caption{A reminder of the commit graph}(\#fig:localCommitGraphAfterPushRejectedAndFetchNoHistory-fig)
\end{figure}
...
\caption{local commit graph after reset}(\#fig:localCommitGraphRebaseExampleAfterResetMasterNoHistory-fig)
\end{figure}
...
\caption{Resulting commit graph after feature branch rebasing}(\#fig:localCommitGraphRebaseExampleAfterFBRebaseNoHistory-fig)
\end{figure}
...
\caption{Reorganised history}(\#fig:localCommitGraphRebaseExampleCompleteNoHistory-fig)
\end{figure}
...
\caption{local commit graph after push}(\#fig:localCommitGraphRebaseExampleAfterPushNoHistory-fig)
\end{figure}
...
\caption{Our code history on GitLab is pleasantly clear and linear}(\#fig:commitGraphInGitLabRebaseExampleAfterPushNoHistory-fig)
\end{figure}
...
\caption{The main Jenkins dashboard}(\#fig:dashboard-fig)
\end{figure}
...
\caption{(ref:captionweather)}(\#fig:weather-montage-fig)
\end{figure}
...
\caption{Hovering over an icon will display a tool tip that gives more information about what the icon means in terms of the build.}(\#fig:toolTipWeatherExercise1-fig)
\end{figure}
...
\caption{An example of how a teams coursework builds look in Jenkins}(\#fig:sem1-build-jobs-fig)
\end{figure}
...
\caption{An example of the contents of the Exercise 1 Builds folder displayed in Jenkins.}(\#fig:contentsOfExercise1FolderOnJenkins-fig)
\end{figure}
...
\caption{(ref:captionbuildstatus)}(\#fig:build-status-montage-fig)
\end{figure}
...
\caption{Jenkins view of the master branch}(\#fig:master-branch-fig)
\end{figure}
...
\caption{Jenkins view of the feature branch builds}(\#fig:FeatureBranchBuilds-fig)
\end{figure}
...
\caption{The Issue Revealing Builds folder in Jenkins}(\#fig:IssueRevealingBuilds-fig)
\end{figure}
...
\caption{(ref:captionxkcdquality)}(\#fig:xkcd-code-quality-fig)
\end{figure}
...
\caption{Sample conversation from nodejs}(\#fig:nodejs1-fig)
\end{figure}
\begin{figure}
...
\caption{Sample conversation from nodejs}(\#fig:nodejs2-fig)
\end{figure}
...
\caption{Sample conversation from nodejs}(\#fig:nodejs3-fig)
\end{figure}
...
\caption{Sample conversation from nodejs}(\#fig:nodejs4-fig)
\end{figure}
...
\caption{Sample conversation from nodejs}(\#fig:merge-request-assignee-fig)
\end{figure}
...
\caption{Sample conversation from nodejs}(\#fig:comment-commit-fig)
\end{figure}
